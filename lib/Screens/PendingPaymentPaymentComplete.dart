...
class PaymentPagePendingPayment extends StatefulWidget {
  PaymentPagePendingPayment({Key key,this.edit,this.payment,this.id});
  int edit;
  int payment;
  int id;

  @override
  _PaymentPagePendingPaymentState createState() => _PaymentPagePendingPaymentState();
}

class _PaymentPagePendingPaymentState extends State<PaymentPagePendingPayment> {
  SaveModel li11;
  ...
  var stringlist3=["Payment Mode","Cash","Card"];
  SaveResponse li6;
...
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    <IN_MOB_GETITEMMASTER xmlns="http://tempuri.org/">
      <BranchID>1</BranchID>
      <CategoryID>${li8.details[0].categoryID}</CategoryID>
      <UserID>1</UserID>
...
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    <IN_MOB_INSERT_ORDER xmlns="http://tempuri.org/">
      ...
      <OrderPrice>${li8.details[0].orderPrice}</OrderPrice>
      <AdvanceType>$dropdownValue1</AdvanceType>
      <AdvanceAmount>${AdvanceController.text}</AdvanceAmount>
          ...
          child: Column(
            children: [
                    ...
                    onChanged: (String newValue) {
                      setState(() {
                        ...
                        if(newValue=="Full/Balance Amount") {
                          advenable=false;
                          AdvanceController.text = ((li8.details[0].orderPrice)-(li8.details[0].advanceAmount)).toStringAsFixed(2);
                        }else {
                          advenable=true;
              ...
              ExpansionTile(
                ...
                children: [
                  widget.edit!=0?
                  ListTile(title: Text("Order Time and Date"),trailing: Text("${(DateFormat("hh:mm a , dd-MM-yyyy")).format(DateTime.fromMillisecondsSinceEpoch(int.parse(li8.details[0].bookingDate.toString().replaceAll("/Date(", "").replaceAll(")/", "")))) }")
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                                flex: 1,
                                child: Text(
                                  "Rs.${li8.details[0].advanceAmount.toStringAsFixed(2) }",
                                  textAlign: TextAlign.start,
                                )),
                          ],
                        ),
          ...
          if (widget.edit == 0) {
            ...
            if (widget.payment == 0) {
              if ((dropdownValue1 != "Advance Type") &&
                  (dropdownValue2 != "Payment Mode") &&
                  (AdvanceController.text.length != 0) &&
                  (double.parse(AdvanceController.text) <= li8.details[0].orderPrice-li8.details[0].advanceAmount))
