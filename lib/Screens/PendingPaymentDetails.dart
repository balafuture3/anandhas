...
import 'PendingPaymentPaymentComplete.dart';

class PendingPaymentDetails extends StatefulWidget {
  PendingPaymentDetails({Key key, this.orderid, this.gst, this.invoice});
  String orderid;
  bool gst;
  String invoice;
  @override
  PendingPaymentDetailsState createState() => PendingPaymentDetailsState();
}

class PendingPaymentDetailsState extends State<PendingPaymentDetails> {
  bool loading = false;
          ...
          return  [
            ...
            pw.Container(
                ...
                child: pw.Row(
                    mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                    children: [
                      li8.details[0].inVoiceNo==""?
                      pw.Expanded(
                          flex: 1,
                          child: pw.Text("Invoice No : ORDNO${widget.orderid}/${li6.sTATUSMSG}"))
                          :pw.Expanded(
                          flex: 1,
            ...
            pw.Row(children: [
              pw.Padding(
                ...
                child: pw.Text(
                  "Name: ${PendingPaymentDetailsState.li8.details[0].name.toString()}",
                  softWrap: true,
            ...
            pw.Row(children: [
              pw.Padding(
                ...
                child: pw.Text(
                  "Mobile: ${PendingPaymentDetailsState.li8.details[0].mobile.toString()}",
                  softWrap: true,
  ...
  Future<void> shareInvoice() async {
    ...
    //   build: (format) => pdf.save(),
    // );
    await Printing.sharePdf(bytes: await pdf.save(), filename: 'Discount ${widget.invoice}.pdf');
    // await Printing.layoutPdf(
    //   format: PdfPageFormat.a4,
    //   onLayout: (PdfPageFormat format) async => pdf.save(),
    //
  ...
  Future<void> shareInvoiceCancel() async {
    final pdf = pw.Document();
...
//    print(String_values.base_url);
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        datefromcontroller.text= "${(DateFormat("dd-MM-yyyy")).format(DateTime.fromMillisecondsSinceEpoch(int.parse(PendingPaymentDetailsState.li8.details[0].bookingDate.toString().replaceAll("/Date(", "").replaceAll(")/", "")))) }";
        timecontroller.text= "${(DateFormat("hh:mm a")).format(DateTime.fromMillisecondsSinceEpoch(int.parse(PendingPaymentDetailsState.li8.details[0].bookingDate.toString().replaceAll("/Date(", "").replaceAll(")/", "")))) }";
        timeupload="${(DateFormat("hh:mm:ss")).format(DateTime.fromMillisecondsSinceEpoch(int.parse(PendingPaymentDetailsState.li8.details[0].bookingDate.toString().replaceAll("/Date(", "").replaceAll(")/", "")))) }";
        dateupload ="${(DateFormat("yyyy-MM-dd")).format(DateTime.fromMillisecondsSinceEpoch(int.parse(PendingPaymentDetailsState.li8.details[0].bookingDate.toString().replaceAll("/Date(", "").replaceAll(")/", "")))) }";
      });
  ...
  Future<http.Response> InvoiceUpdate() async {
    setState(() {
      loading = true;
    });
    var envelope = '''
                    ...
                    Column(
                      children: [
                        ...
                        ListTile(
                            ...
                            trailing: Text("${(DateFormat("hh:mm a , dd-MM-yyyy")).format(DateTime.fromMillisecondsSinceEpoch(int.parse(PendingPaymentDetailsState.li8.details[0].bookingDate.toString().replaceAll("/Date(", "").replaceAll(")/", "")))) }")
                        )
